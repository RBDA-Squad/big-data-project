...
public class LonLatToZipCodeReducer extends Reducer<Text, Text, Text, Text> {
  ...
  public void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
    ...
    Path path = new Path("hdfs://dumbo/user/syc574/FinalProject/output3/part-r-00000");
    ...
    for (Text value : values) {
      ...
      //context.write(key, new Text(Double.toString(min_n)));
      String min_zipcode = "";
      ...
      try {
        ...
        while ((line_tmp = br.readLine()) != null) {
          ...
          s = 2 * Math.asin(Math.sqrt(Math.pow(Math.sin(dlat/2),2) + Math.cos(radLat1)*Math.cos(radLat2)*Math.pow(Math.sin(dlon/2),2)));
          s = s * 6378.137;
          s = Math.round(s * 10000) / 10000;
          
          count += 1;
          //tmp_use = Integer.toString(count) + "," + items2[0] + "," + items2[1] + "," + items2[2] + "," + min_zipcode;
          //context.write(new Text(Double.toString(s)), new Text(tmp_use));
          if (s < min_n) { 
            min_n = s;
            min_zipcode = zipcode;
            tmp_use = min_zipcode;
